...
/*   Updated: 2024/11/11 22:28:29 by florian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_world	*init_world(void)
{
	...
	world->light = create_light(create_point(-10, 10, -10), create_point(1, 1,
				1));
	return (world);
}
...
t_array	*intersect_world(t_ray ray, t_world *world)
{
	...
	while (current != NULL)
	{
		intersections = concatenate_array(intersections,
				sphere_intersections(ray, current));
...
t_tuple	color_at(t_world *world, t_ray ray)
{
	...
	comps = prepare_computations(intersections->array[hit_index(intersections,
				hit_array(intersections))], ray);
...
t_4matrix	view_transform(t_tuple from, t_tuple to, t_tuple up)
{
	...
	return (mat_product(orientation, translation_mat(-from.x, -from.y,
				-from.z)));
}
...
t_ray	ray_for_pixel(t_camera camera, double px, double py)
{
	...
	pixel = mat_tuple_product(inverse4(camera.transform), create_point(world_x,
				world_y, -1));
	origin = mat_tuple_product(inverse4(camera.transform), create_point(0, 0,
				0));
