...
/*   Updated: 2024/10/14 16:07:57 by florian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple lighting(t_material material, t_light light, t_tuple point, t_tuple eye_vector, t_tuple normal_vector)
{
   ...
   t_tuple lightv = normalize_vector(substract_floats(light.position, point));
   t_tuple ambient = multiply_color(effective_color, material.ambient);
   
   float light_dot_normal = scalar_product(lightv, normal_vector);
   t_tuple diffuse;
   t_tuple specular;
   if (light_dot_normal < 0)
   {
    ...
    else
    {
        ...
        t_tuple reflectv = sphere_reflect(multiply_tuple(lightv, -1), normal_vector);
        float reflect_dot_eye = scalar_product(reflectv, eye_vector);
